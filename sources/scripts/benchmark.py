...
def run_benchmark(config_path: Path, cache_db: Path) -> None:
    ...
    if not result_warm.statistics.cache_hit:
        print(f"Validation Reason: {result_warm.cache_validation.reason.value if result_warm.cache_validation else 'NONE'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=Path, default=Path("config/sources.yaml"), help="Path to sources.yaml")
    parser.add_argument("--cache", type=Path, default=Path(".aiodoo_cache/sources.sqlite"), help="Path to test cache DB")
