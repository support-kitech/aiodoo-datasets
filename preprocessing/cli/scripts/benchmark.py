...
def run_benchmark():
    ...
    res_cold = manager.normalize(
        source_context, PipelineOptions(force_reprocess=True, skip_cache=False)
    )
    ...
    t0 = time.perf_counter()
    res_warm = manager.normalize(
        source_context, PipelineOptions(force_reprocess=False, skip_cache=False)
    )
