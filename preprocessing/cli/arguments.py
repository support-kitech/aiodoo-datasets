...
def parse_arguments() -> CliArgs:
    ...
    # validate command
    val_parser = subparsers.add_parser("validate", help="Run Stage 1 & 2 validation without caching")
    ...
    # Global options
    parser.add_argument("--force", action="store_true", help="Force reprocessing even if cache hits")
    parser.add_argument("--skip-cache", action="store_true", help="Completely bypass cache read/writes")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging and stack traces")
    parser.add_argument("--quiet", action="store_true", help="Suppress non-error output")
    parser.add_argument("--json", action="store_true", dest="json_output", help="Output purely structured JSON")
