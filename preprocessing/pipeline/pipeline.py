    ...
    def execute(self, p_context: PipelineContext) -> PipelineResult:
        ...
        try:
            ...
            context_hash = hashlib.md5(
                str(
                    [r.name + str(r.version) for r in p_context.source_context.repositories]
                ).encode()
            ).hexdigest()
            cache_key = p_context.cache_key or CacheKey(source_context_hash=context_hash)
            cached_data = (
                self._cache_store.get(cache_key) if not p_context.options.skip_cache else None
            )
            ...
            if cached_data and not p_context.options.force_reprocess:
                ...
                if invalidation.is_valid:
                    ...
                    stats = PipelineStatistics(
                        ...
                        files_processed=sum(
                            len(m.files)
                            for r in deserialized_context.repositories
                            for m in r.modules
                        ),
                    ...
                    return PipelineResult(
                        success=True, context=deserialized_context, statistics=stats
                    )
            ...
            for source_repo in p_context.source_context.repositories:
                ...
                for source_module in source_repo.modules:
                    ...
                    all_files = [
                        f
                        for f in source_module.path.rglob("*")
                    ...
                    module = RepositoryBuilder.build_module(
                        source_module.name, tuple(preprocessed_files), metadata
                    )
            ...
            final_context = ContextBuilder.build(
                p_context.source_context, tuple(preprocessed_repos)
            )
            ...
            if not p_context.options.skip_cache:
                ...
                cache_meta = CacheMetadata(
                    ...
                    processor_registry_hash=hashlib.sha256(
                        str(self._registry).encode()
                    ).hexdigest(),
            ...
            return PipelineResult(success=True, context=final_context, statistics=stats)
