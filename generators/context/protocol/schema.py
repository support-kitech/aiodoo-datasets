...
class ProtocolArtifact(BaseModel):
    ...
    ranking_reason: ProtocolRankingReason = Field(
        ..., description="Canonical reason the artifact was ranked."
    )
...
class ProtocolGraph(BaseModel):
    ...
    nodes: list[ProtocolNode] = Field(
        default_factory=list, description="Sorted list of minimal nodes."
    )
    edges: list[ProtocolEdge] = Field(
        default_factory=list, description="Sorted list of minimal edges."
    )
...
class ProtocolMetadata(BaseModel):
    ...
    relationship_count: int = Field(
        ..., description="Number of relationships in the minimal graph."
    )
...
class ContextTask(BaseModel):
    ...
    artifacts: list[ProtocolArtifact] = Field(
        default_factory=list, description="Ranked artifacts sorted by score and ID."
    )
