...
class ContextMapper:
    ...
    def map(
        self, query: Query, results: list[RankingResult], graph: ContextGraph, protocol_hash: str
    ) -> ContextTask:
        """
        ...
        try:
            ...
            for result in results:
                ...
                if matched_relationship:
                    ...
                    for edge in graph.find_edges_by_type(matched_relationship):
                        if (
                            edge.source_id == result.node_id and edge.target_id == query.target_node
                        ) or (
            ...
            proto_graph = ProtocolGraph(nodes=sorted_nodes, edges=sorted_edges)
