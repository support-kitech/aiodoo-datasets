...
class ContextPipeline:
    ...
    def __init__(
        ...
        self.writer = DatasetWriter(self.output_dir, self.stats, "context_v1_0.jsonl", "aiodoo_context")
    ...
    def run(self) -> None:
        ...
        if self.resume:
            ...
            modules_to_process = [m for m in all_modules if not self.checkpoint.is_module_fully_processed(m.name)]
            logger.info("Resuming. Processing %d out of %d modules.", len(modules_to_process), len(all_modules))
        else:
            self.checkpoint.clear()
            modules_to_process = all_modules
            
        with ProcessPoolExecutor(max_workers=self.workers) as executor:
            future_to_module = {executor.submit(process_module, mod): mod for mod in modules_to_process}
            ...
            for future in as_completed(future_to_module):
                ...
                try:
                    ...
                    for task in tasks:
                        ...
                        if self.resume and self.checkpoint.is_processed("odoo", mod.name, "context_task", task.id):
                            continue
                        ...
                        self.checkpoint.save("odoo", mod.name, "context_task", task.id, self.writer.written_count)
                    ...
                    self.checkpoint.save("odoo", mod.name, "module_completion", "done", self.writer.written_count)
