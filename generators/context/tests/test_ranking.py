...
from aiodoo_datasets.generators.context.ranking.rules import (
    DefinitionRule, InheritanceRule, DependencyRule, ViewRule, SecurityRule, ActionRule
)
...
class TestRankingRules(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.n_custom_model = ContextNode("sale.order", "custom_sale", "models/sale_order.py", NodeType.MODEL, LanguageType.PYTHON)
        ...
        for n in [self.n_model, self.n_custom_model, self.n_manifest_sale, self.n_manifest_stock, self.n_view, self.n_field, self.n_acl, self.n_action]:
            self.graph.add_node(n)
            
        self.graph.add_edge(ContextEdge(self.n_custom_model.node_id, self.n_model.node_id, RelationshipType.INHERITS))
        self.graph.add_edge(ContextEdge(self.n_manifest_stock.node_id, self.n_manifest_sale.node_id, RelationshipType.DEPENDS))
        self.graph.add_edge(ContextEdge(self.n_view.node_id, self.n_field.node_id, RelationshipType.DISPLAYS))
        self.graph.add_edge(ContextEdge(self.n_acl.node_id, self.n_model.node_id, RelationshipType.SECURES))
        self.graph.add_edge(ContextEdge(self.n_action.node_id, self.n_view.node_id, RelationshipType.TRIGGERS))
        
    def test_definition_rule(self):
        rule = DefinitionRule()
        query = Query(QueryType.FIND_MODEL, QueryIntent.FIND_MODEL, self.n_model.node_id, "sale.order", "NL")
    ...
    def test_dependency_rule(self):
        rule = DependencyRule()
        query = Query(QueryType.FIND_DEPENDENCY, QueryIntent.FIND_DEPENDENCY, self.n_manifest_sale.node_id, "sale", "NL")
        results = rule.rank(query, self.graph)
        self.assertEqual(len(results), 1)
    ...
    def test_view_rule(self):
        rule = ViewRule()
        query = Query(QueryType.FIND_VIEW, QueryIntent.FIND_VIEW, self.n_field.node_id, "amount_total", "NL")
        results = rule.rank(query, self.graph)
        self.assertEqual(len(results), 1)
    ...
    def test_security_rule(self):
        rule = SecurityRule()
        query = Query(QueryType.FIND_SECURITY, QueryIntent.FIND_SECURITY, self.n_model.node_id, "sale.order", "NL")
        results = rule.rank(query, self.graph)
        self.assertEqual(len(results), 1)
    ...
    def test_action_rule(self):
        rule = ActionRule()
        query = Query(QueryType.FIND_ACTION, QueryIntent.FIND_ACTION, self.n_view.node_id, "view_sale_order_form", "NL")
        results = rule.rank(query, self.graph)
        self.assertEqual(len(results), 1)
