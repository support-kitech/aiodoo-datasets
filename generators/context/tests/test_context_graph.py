...
from aiodoo_datasets.generators.context.analysis.graph import (
    ContextNode, ContextEdge, ContextGraph,
    NodeType, LanguageType, RelationshipType
)
...
class TestContextGraph(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node1 = ContextNode(
            name="node1", module="base", relative_path="a.py",
            node_type=NodeType.MODEL, language=LanguageType.PYTHON, start_line=1
        )
        self.node2 = ContextNode(
            name="node2", module="base", relative_path="b.py",
            node_type=NodeType.FIELD, language=LanguageType.PYTHON, start_line=2
        )
        self.edge1 = ContextEdge(
            source_id=self.node1.node_id, target_id=self.node2.node_id,
            relationship_type=RelationshipType.CONTAINS
        )
    ...
    def test_deterministic_ordering(self):
        ...
        n_z = ContextNode(name="z", module="base", relative_path="z.py", node_type=NodeType.MODEL, language=LanguageType.PYTHON)
        n_a = ContextNode(name="a", module="base", relative_path="a.py", node_type=NodeType.MODEL, language=LanguageType.PYTHON)
        n_m = ContextNode(name="m", module="base", relative_path="m.py", node_type=NodeType.MODEL, language=LanguageType.PYTHON)
