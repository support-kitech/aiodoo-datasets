...
class TestContextMapper(unittest.TestCase):
    def setUp(self):
        ...
        self.n_artifact = ContextNode(
            "sale.order", "custom_sale", "models/custom.py", NodeType.MODEL, LanguageType.PYTHON
        )
        self.n_unrelated = ContextNode(
            "stock.picking", "stock", "models/stock.py", NodeType.MODEL, LanguageType.PYTHON
        )
        ...
        self.edge1 = ContextEdge(
            self.n_artifact.node_id, self.n_target.node_id, RelationshipType.INHERITS
        )
        self.edge2 = ContextEdge(
            self.n_unrelated.node_id, self.n_target.node_id, RelationshipType.DEPENDS
        )
        self.graph.add_edge(self.edge1)
        self.graph.add_edge(self.edge2)

        self.query = Query(
            QueryType.FIND_MODEL,
            QueryIntent.FIND_MODEL,
            self.n_target.node_id,
            "sale.order",
            "Where?",
        )
        ...
        self.results = [
            RankingResult(
                ...
                metadata=freeze_metadata(
                    {"start_line": 10, "matched_relationship": RelationshipType.INHERITS.value}
                ),
            )
        ]
