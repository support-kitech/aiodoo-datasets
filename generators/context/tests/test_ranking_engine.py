...
class MockMultiRule1(BaseRankingRule):
    ...
    supported_query_types = [QueryType.FIND_MODEL]
    def rank(self, query, graph):
        return [RankingResult("q1", "node_a", RankingScore.DEFINITION, self.rule_type, RankingReason.DIRECT_DEFINITION, metadata=freeze_metadata({"start_line": 10}))]
...
class MockMultiRule2(BaseRankingRule):
    ...
    def rank(self, query, graph):
        ...
        return [RankingResult("q1", "node_a", RankingScore.INHERITANCE, self.rule_type, RankingReason.MODEL_INHERITANCE, metadata=freeze_metadata({"start_line": 10}))]
...
class MockMultiRuleAction(BaseRankingRule):
    ...
    def rank(self, query, graph):
        ...
        return [RankingResult("q1", "node_a", RankingScore.ACTION, self.rule_type, RankingReason.ACTION_REFERENCE, metadata=freeze_metadata({"start_line": 10}))]
...
class MockTieRule1(BaseRankingRule):
    ...
    def rank(self, query, graph):
        ...
        return [
            RankingResult("q1", "node_z", RankingScore.ACTION, self.rule_type, RankingReason.DIRECT_DEFINITION, metadata=freeze_metadata({"relative_path": "b", "start_line": 20})),
            RankingResult("q1", "node_a", RankingScore.ACTION, self.rule_type, RankingReason.DIRECT_DEFINITION, metadata=freeze_metadata({"relative_path": "a", "start_line": 10}))
        ]

class TestRankingEngine(unittest.TestCase):
    
    def setUp(self):
        logging.getLogger('aiodoo_datasets.generators.context.ranking.ranking_engine').setLevel(logging.CRITICAL)
