...
class DecisionValidator:
    ...
    def validate(review: Review) -> Tuple[str, ...]:
        ...
        if review.decision.status == DecisionEnum.APPROVED:
            critical_findings = [
                f for f in review.findings if f.severity == Severity.CRITICAL and not f.is_positive
            ]
            if critical_findings:
                diagnostics.append(
                    f"Review is APPROVED but contains {len(critical_findings)} CRITICAL findings."
                )
