...
class RuleRegistry:
    ...
    def register(cls, rule_class: Type[BaseRule]) -> None:
        ...
        if not hasattr(rule_class, "RULE_CATEGORY") or not isinstance(rule_class.RULE_CATEGORY, RuleCategory):
        ...
        if priority in cls._priorities:
            raise ValidationException(f"Duplicate priority {priority} for rules {cls._priorities[priority]} and {rule_id}")
