...
def build_repair_task(opp: RepairOpportunity, base_path: Path) -> RepairTask:
    ...
    return RepairTask(
        id=opp.id,
        problem=Problem(
            description=opp.problem_description, severity=opp.severity, location=opp.location
        ),
        root_cause=RootCause(analysis=opp.root_cause),
        context=[artifact],
        artifacts=[artifact],
        expected_outcome=ExpectedOutcome(operations=opp.operations, explanation=opp.explanation),
        constraints=[],
        metadata={
            "rule_id": opp.rule_id,
