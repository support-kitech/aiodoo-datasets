    ...
    def add_sample(self, record: RepairDatasetRecord, json_str: str) -> None:
        ...
        if hasattr(record.output, "tasks"):
            for task in record.output.tasks:
                ...
                # Severity
                severity = task.problem.severity.lower() if hasattr(task.problem, "severity") else "low"
                ...
                # Operations
                if hasattr(task, "expected_outcome") and hasattr(task.expected_outcome, "operations"):
                    self.total_operations += len(task.expected_outcome.operations)

    def get_manifest_data(self) -> dict[str, Any]:
        return {
            "average_tasks_per_sample": round(self.total_opportunities_found / max(1, self.total_samples), 2),
            "average_operations_per_task": round(self.total_operations / max(1, self.total_opportunities_found), 2)
        }
