...
def compute_protocol_hash(module: OdooModule, scenario: Scenario, payload: ArtifactPayload) -> str:
    ...
    canonical_repr = {
        ...
        "artifacts": sorted([{"id": a.id, "type": a.type, "path": a.path, "intent": a.intent, "deps": sorted(a.dependencies)} for a in payload.artifacts], key=lambda x: x["path"]),
        "operations": sorted([{"op": o.operation, "path": o.path} for o in payload.operations], key=lambda x: x["path"]),
        "validations": sorted([{"action": v.action, "reason": v.reason} for v in payload.validation_actions], key=lambda x: (x["action"], x["reason"]))
    }
    json_str = json.dumps(canonical_repr, sort_keys=True, separators=(',', ':'))
    return hashlib.sha256(json_str.encode("utf-8")).hexdigest()

def build_metadata(module: OdooModule, scenario: Scenario, payload: ArtifactPayload) -> dict[str, Any]:
