...
class CodingStatistics(BaseStatistics):
    ...
    def add_sample(self, record: CodingDatasetRecord, json_str: str) -> None:
        ...
        if record.context:
            self.total_context_size += len(record.context.get("existing_models", [])) + len(record.context.get("existing_views", []))
    ...
    def get_export_stats(self) -> dict[str, Any]:
        return {
            ...
            "average_artifacts_per_sample": round(self.total_artifacts / max(1, self.total_samples), 2),
            "average_operations_per_sample": round(self.total_operations / max(1, self.total_samples), 2),
            "average_dependencies_per_artifact": round(self.total_dependencies / max(1, self.total_artifacts), 2),
