...
def build_validation_actions(artifacts: list[GeneratedArtifact], py_k: PythonKnowledge, xml_k: XMLKnowledge, module: OdooModule) -> list[ValidationAction]:
    """
    ...
    if has_python:
        actions.append(ValidationAction(
            action="Restart Odoo",
            reason="Python logic modified, requires server restart."
        ))
        
    if has_xml:
        actions.append(ValidationAction(
            action="Update Module",
            reason="XML logic modified, requires UI update."
        ))
