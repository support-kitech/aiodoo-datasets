...
def build_operations(artifacts: list[GeneratedArtifact], py_k: PythonKnowledge, xml_k: XMLKnowledge) -> list[FileOperation]:
    """
    ...
    for artifact in artifacts:
        ...
        if path.endswith(".py") and path in py_k.files:
            ...
            # Manifest
            elif "depends" in getattr(file_info, "manifest", {}) or path.endswith("__manifest__.py") or path.endswith("__openerp__.py"):
                op_type = "UPDATE"
            ...
            if not is_patch and hasattr(file_info, "tags") and any("menuitem" in t or "act_window" in t for t in file_info.tags):
                is_patch = True
