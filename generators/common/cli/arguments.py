...
def build_base_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the sources.yaml defining Odoo repositories",
    )
    parser.add_argument(
        "--output",
        type=str,
        required=True,
        help="Directory to output the dataset, cache, and checkpoints",
    )
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of concurrent multiprocessing workers"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume pipeline generation from the last saved checkpoint",
    )
    parser.add_argument(
        "--reset-checkpoint",
        action="store_true",
