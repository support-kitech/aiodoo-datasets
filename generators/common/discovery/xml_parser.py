...
class OdooXMLParser:
    ...
    def parse_module(self, module_path: Path):
        ...
        if any([
            global_knowledge.views, global_knowledge.actions, global_knowledge.menus, 
            global_knowledge.security_rules, global_knowledge.assets, global_knowledge.data_records
        ]):
        ...
        try:
            ...
            for record in root.findall(".//record"):
                ...
                if model == "ir.ui.view":
                    ...
                    knowledge.views.append(OdooViewDef(
                        id=record_id, model=model_name, view_type=view_type, inherit_id=inherit_id
                    ))
                    
                elif model and model.startswith("ir.actions."):
                    res_model = self._extract_field(record, "res_model")
                    knowledge.actions.append(OdooActionDef(
                        id=record_id, action_type=model, model=res_model
                    ))
                    
                elif model == "ir.rule":
                    model_id = self._extract_field(record, "model_id")
                    knowledge.security_rules.append(OdooSecurityDef(
                        id=record_id, model=model_id, type="ir.rule"
                    ))
                elif model == "res.groups":
                    knowledge.security_rules.append(OdooSecurityDef(
                        id=record_id, type="res.groups"
                    ))
                elif model:
                    knowledge.data_records.append(OdooDataRecordDef(
                        id=record_id, model=model
                    ))

            for menu in root.findall(".//menuitem"):
                knowledge.menus.append(OdooMenuDef(
                    id=menu.get("id", ""),
                    name=menu.get("name", ""),
            ...
            for template in root.findall(".//template"):
                ...
                if inherit_id and "assets_" in inherit_id:
                    knowledge.assets.append(OdooAssetDef(
                        id=template.get("id", ""),
