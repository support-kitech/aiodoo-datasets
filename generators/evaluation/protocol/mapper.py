...
class ProtocolMapper:
    ...
    def _map_metadata(metadata: EvaluationMetadata) -> MetadataProtocol:
        return MetadataProtocol(
            ...
            evaluation_type=metadata.evaluation_type.value
            if hasattr(metadata.evaluation_type, "value")
            else metadata.evaluation_type,
            difficulty=metadata.difficulty.value
            if hasattr(metadata.difficulty, "value")
    ...
    def _map_benchmark_metadata(metadata: BenchmarkMetadata) -> BenchmarkMetadataProtocol:
        return BenchmarkMetadataProtocol(
            ...
            benchmark_category=metadata.benchmark_category.value
            if hasattr(metadata.benchmark_category, "value")
            else metadata.benchmark_category,
    ...
    def _map_case(case: EvaluationCase) -> EvaluationCaseProtocol:
        return EvaluationCaseProtocol(
            ...
            success_criteria=tuple(
                ProtocolMapper._map_success_criteria(sc) for sc in case.success_criteria
            ),
            failure_criteria=tuple(
                ProtocolMapper._map_failure_criteria(fc) for fc in case.failure_criteria
            ),
    ...
    def _map_failure_criteria(criteria: FailureCriteria) -> FailureCriteriaProtocol:
        return FailureCriteriaProtocol(
            criteria_id=criteria.criteria_id, description=criteria.description, fatal=criteria.fatal
        )
