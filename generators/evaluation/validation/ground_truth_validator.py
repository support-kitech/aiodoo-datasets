...
class GroundTruthValidator:
    ...
    def validate(ground_truth: GroundTruth, expected_output: ExpectedOutput) -> None:
        ...
        if not ground_truth.ground_truth_id.startswith("TRUTH-"):
            raise EvaluationValidationError(f"Invalid GroundTruth ID prefix: {ground_truth.ground_truth_id}")
            
        if not expected_output.output_id.startswith("EXPOUT-"):
            raise EvaluationValidationError(f"Invalid ExpectedOutput ID prefix: {expected_output.output_id}")
