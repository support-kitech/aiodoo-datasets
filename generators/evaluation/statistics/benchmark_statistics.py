...
class BenchmarkStatistics:
    ...
    def compute(dataset: Tuple[Any, ...]) -> MappingProxyType:  # type: ignore[type-arg]
        ...
        return MappingProxyType(
            {
                "coverage_by_generator": MappingProxyType(
                    dict(sorted(coverage_by_generator.items()))
                ),
                "coverage_by_protocol": MappingProxyType(
                    dict(sorted(coverage_by_protocol.items()))
                ),
                "coverage_by_odoo_version": MappingProxyType(
                    dict(sorted(coverage_by_odoo_version.items()))
                ),
                ...
                "coverage_by_difficulty": MappingProxyType(
                    dict(sorted(coverage_by_difficulty.items()))
                ),
                "coverage_by_complexity": MappingProxyType(
                    dict(sorted(coverage_by_complexity.items()))
                ),
                "category_distribution": MappingProxyType(
                    dict(sorted(category_distribution.items()))
                ),
