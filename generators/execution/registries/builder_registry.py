    ...
    def validate(self) -> None:
        ...
        for builder in self._items:
            ...
            if builder.PRIORITY in priorities:
                raise ValueError(f"Duplicate PRIORITY {builder.PRIORITY} found in {builder.__class__.__name__}")
        ...
        for builder in self._items:
            if hasattr(builder, "REQUIRES"):
                for req in builder.REQUIRES:
                    if req not in types:
                        raise ValueError(f"{builder.__class__.__name__} REQUIRES {req.__name__}, but it is not registered.")
                    if req not in seen:
                        raise ValueError(f"{builder.__class__.__name__} REQUIRES {req.__name__}, but it has a higher PRIORITY (runs later).")
