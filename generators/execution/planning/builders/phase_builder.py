...
class PhaseBuilder:
    ...
    def build(context: PlanningContext, stage_result: StageResult) -> PhaseResult:
        ...
        if not stage_result.success:
            return PhaseResult(
                success=False, diagnostics=("Failed to build phases due to invalid stage result.",)
            )

        phase = ExecutionPhase(phase_id="phase_1", name="Main Phase", stages=stage_result.stages)
