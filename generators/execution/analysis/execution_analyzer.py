...
class ExecutionAnalyzer:
    ...
    def execute(self, context: AnalysisContext) -> ExecutionKnowledge:
        ...
        return ExecutionKnowledge(
            artifacts=results.get("ArtifactAnalyzer").artifacts if "ArtifactAnalyzer" in results else (),
            operations=results.get("OperationAnalyzer").operations if "OperationAnalyzer" in results else (),
            dependencies=results.get("DependencyAnalyzer").dependencies if "DependencyAnalyzer" in results else (),
            constraints=results.get("ConstraintAnalyzer").constraints if "ConstraintAnalyzer" in results else (),
            verifications=results.get("VerificationAnalyzer").verifications if "VerificationAnalyzer" in results else (),
            rollbacks=results.get("RollbackAnalyzer").rollbacks if "RollbackAnalyzer" in results else ()
        )
