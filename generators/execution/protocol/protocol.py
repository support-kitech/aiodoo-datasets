...
class Protocol:
    ...
    def map_protocol(context: ProtocolContext) -> ProtocolResult:
        ...
        if not validation_result.success:
            return ProtocolResult(success=False, protocol=mapper_result.protocol, diagnostics=validation_result.violations)
        ...
        if not serializer_result.success:
            return ProtocolResult(success=False, protocol=mapper_result.protocol, diagnostics=serializer_result.diagnostics)
