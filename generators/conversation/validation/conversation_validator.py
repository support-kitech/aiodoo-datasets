...
from aiodoo_datasets.generators.conversation.validation.reference_validator import ReferenceValidator
...
class ConversationValidator:
    ...
    def validate(conversation: Conversation) -> None:
        if not conversation.conversation_id.startswith("CONV-"):
            raise ConversationValidationError(f"Invalid conversation ID format: {conversation.conversation_id}")
            
        if not conversation.turns:
            raise ConversationValidationError(f"Conversation {conversation.conversation_id} has no turns.")
